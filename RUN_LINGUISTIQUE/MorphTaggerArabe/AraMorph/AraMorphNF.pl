...
use File::Spec;
use File::Basename;
$BAMADIR = dirname( File::Spec->rel2abs( __FILE__ ) );

# load 3 compatibility tables (load these first so when we load the lexicons we can check for undeclared $cat values)
%hash_AB = load_table("$BAMADIR/tableAB"); # load compatibility table for prefixes-stems    (AB)
%hash_AC = load_table("$BAMADIR/tableAC"); # load compatibility table for prefixes-suffixes (AC)
%hash_BC = load_table("$BAMADIR/tableBC"); # load compatibility table for stems-suffixes    (BC)

# load 3 lexicons
%prefix_hash = load_dict("$BAMADIR/dictPrefixes"); # dict of prefixes (A)
%stem_hash   = load_dict("$BAMADIR/dictStems");    # dict of stems    (B)
%suffix_hash = load_dict("$BAMADIR/dictSuffixes"); # dict of suffixes (C)
my $sennum = 0;
while (<STDIN>) {
...
#   print STDERR "reading input line $.\r";
   #empty lines should be kept for allignments...
   #if (/^\s*$/) {
	#print STDERR "\nEMPTYLINE $.\n";
        #next;
   #}
...
sub load_dict { # loads a dict into a hash table where the key is $entry and its value is a list (each $entry can have multiple values)
   ...
   while (<IN>) {
      if (m/^;; /) {  
         $lemmaID = $'; 
         #UPDATE 29.8.07, for some reason I have space in the lemmas outputted, remove trailing spaces
         $lemmaID =~ s/\s+$//;
         #old
         #chomp($lemmaID);
