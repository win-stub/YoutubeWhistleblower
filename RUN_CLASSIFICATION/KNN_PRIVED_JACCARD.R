...
m_jaccard1   <- dist.binary(dtm1.new.matrix,method = 1)
m_jaccard1   <- as.matrix(m_jaccard1)
m_jaccard1[is.nan(m_jaccard1)] <- 0
# CONSTRUCT A COSINE MATRIX 2
m_jaccard2   <- dist.binary(dtm2.new.matrix,method = 1)
m_jaccard2   <- as.matrix(m_jaccard2)
m_jaccard2[is.nan(m_jaccard2)] <- 0
# la somme des deux matrix
m.jaccard <- m_jaccard1+m_jaccard2
...
### format IN COSINE
kproche <- 5
sink(paste("/home/saber/DA_Stage/Stage/graph/train_data/",paste('result_kppv.jaccard.test.',kproche,sep='') , sep= ''))
for(index_video in 1:dim(dtm_test.matrix1)[1])
{
  distance_jaccard1 <- c(NULL)
  column_predict <- dtm_test.matrix1[index_video,]#dtm.filter.matrix["126",]
  ### distance cosine
  for(i in 1:length(rownames(dtm1.new.matrix)))
  {
    distance_tmp       <- dist(t(cbind(unlist(dtm1.new.matrix[i,]), unlist(column_predict))), "binary")
    distance_jaccard1  <- c(distance_jaccard1,distance_tmp)
  }
  distance_jaccard1[is.nan(distance_jaccard1)] <- 0
  ### distance cosine
  column_predict <- dtm_test.matrix2[index_video,]#dtm.filter.matrix["126",]
  distance_jaccard2 <- c(NULL)
  for(i in 1:length(rownames(dtm2.new.matrix)))
  {
    distance_tmp       <- dist(t(cbind(unlist(dtm2.new.matrix[i,]), unlist(column_predict))), "binary")
    distance_jaccard2  <- c(distance_jaccard2,distance_tmp)
  }
  distance_jaccard2[is.nan(distance_jaccard2)] <- 0
  #
  distance_jaccard <- distance_jaccard1 +distance_jaccard2
  # KNN FUNCTION
  ...
  vect_knn <- KNN_SN(distance_jaccard, m.jaccard, k=kproche,FALSE)
